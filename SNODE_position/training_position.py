...
model , hist = klax.fit(
    model,
    (time_train , robot_train_input, ball_train[:,0 , :] , ball_train[:,:, :]  ),
    validation_data=(time_test , robot_test_input,ball_test[:,0 , :], ball_test[:, :, :]),
    batch_size=64,
...
print ("true inital value" , true_position[-1,])
print ("pred inital value" , pred_position[-1,])

DT = 0.002
ts = jnp.arange(2500) * DT

ball_q_true , true_time = ball_free_flight_trajecotry( true_position[-1,] , ts)
ball_q_pred , pred_time = ball_free_flight_trajecotry(pred_position [-1,], ts)
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
eqx.tree_serialise_leaves("trained_model_position9.eqx", model_)
